...
class FeedForward(nn.Module):
    def __init__(self, dim,num_frame=5, ffn_expansion_factor=2.66, bias=False):
        super(FeedForward, self).__init__()

        hidden_features = int(dim*ffn_expansion_factor)

        self.project_in = nn.Conv2d(dim*num_frame, hidden_features*2, kernel_size=1, bias=bias)

        self.dwconv = nn.Conv2d(hidden_features*2, hidden_features*2, kernel_size=3, stride=1, padding=1, groups=hidden_features*2, bias=bias)

        self.project_out = nn.Conv2d(hidden_features, dim*num_frame, kernel_size=1, bias=bias)

    def forward(self, x):
        B, D, H, W, C = x.shape
        x = x.permute(0, 4, 1, 2, 3).contiguous().view(B, D*C, H, W)
        x = self.project_in(x)
        x1, x2 = self.dwconv(x).chunk(2, dim=1)
        x = F.gelu(x1) * x2
        x = self.project_out(x)
        x = x.view(B, C, D, H, W).permute(0, 2, 3, 4, 1).contiguous()
        return x
...
class SwinTransformerLayer3D(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = shortcut + self.ffd(self.norm1(x))
